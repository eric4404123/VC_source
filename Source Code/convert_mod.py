...
def get_model(hps_path='vctk.json', model_path='outModel/-127000'):
    hps = Hps()
    hps.load(hps_path)
...
def convert_all_sp(h5_path, src_speaker, tar_speaker, gen=True, 
        dset='test', speaker_used_path='en_speaker_used.txt',
        root_dir='p226_to_p225/',
        model_path='outModel/-127000'):
    # read speaker id file
    with open(speaker_used_path) as f:
        speakers = [line.strip() for line in f]
        speaker2id = {speaker:i for i, speaker in enumerate(speakers)}
    solver = get_model(hps_path='vctk.json', 
...
if __name__ == '__main__':
    h5_path = 'preprocess/RRR/h56.h5'
    root_dir = 'vctk/norm/clf_gen'
